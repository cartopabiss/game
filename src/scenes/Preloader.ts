...
export default class Preloader extends Phaser.Scene {
  ...
  private nomeS: Phaser.GameObjects.BitmapText;
  ...
  init() {
    this._image = this.add
      ...
      .setAlpha(0);
      this._image.setX(this.game.canvas.width / 2 - 5).setScale(1.1)
    ...
    this.nomeS = this.add
    .bitmapText(this.game.canvas.width / 2, 150, "arcade", "", 30)
    .setAlpha(1)
    .setDepth(1001)
    .setOrigin(0.5, 1);
  ...
  loadAssets(): void {
    ...
    this.load.on("complete", () => {
      this.nomeS.setText("ROSSI JAM")
      this._loading.setText("Clicca per giocare")
      this.input.once("pointerdown", () => {
        this.tweens.add({
          targets: [this.nomeS, this._image, this._loading],
          alpha: 0,
          duration: 500,
          onComplete: () => {
